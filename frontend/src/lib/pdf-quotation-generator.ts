...
export async function generateQuotationPDF(quotation: any) {
  ...
  if (quotation.logo_url) {
    try {
      const clientLogoBase64 = await imageUrlToBase64(quotation.logo_url)
      if (clientLogoBase64) {
  ...
  // Right: Business Info from Quotation (Dynamic Client Info)
  doc.setFontSize(8)
  doc.setFont(undefined, "bold")
  doc.setTextColor(0, 0, 0)
  const businessName = quotation.business_name || "PRINCESS JAIDEE ENTERPRISES"
  doc.text(businessName, pageWidth - 15, yPosition, { align: "right" })

  doc.setFontSize(7)
  doc.setFont(undefined, "normal")
  const businessAddress = quotation.business_address || "A.B. Fajardo Bldg., Calle Nueva St.,"
  doc.text(businessAddress, pageWidth - 15, yPosition + 4, { align: "right" })
  
  const businessCity = quotation.business_city || "Brgy. Polvorista, Sorsogon City"
  doc.text(businessCity, pageWidth - 15, yPosition + 7, { align: "right" })
  
  const businessPhone = quotation.business_phone || "0930 821 8871 / 0915 175 9881"
  doc.text(businessPhone, pageWidth - 15, yPosition + 10, { align: "right" })
  
  const businessEmail = quotation.business_email || "pjesorsogonsportswear@gmail.com"
  doc.text(businessEmail, pageWidth - 15, yPosition + 13, { align: "right" })
